...
SAVE_CKPT=9.30_avt_v3_pt_full_wo_maze_latent${LATENT_SIZE}_ce${CE_EMPHASIZE_FACTOR}_align-wt${ALIGN_VISION_LATENT_LOSS_WEIGHT}_emph-wt${EMPHASIZE_LATENT_WEIGHT}
...
torchrun --nproc-per-node=8 --master-port=29501 -m src.main \
  ...
  --log_file "./log.txt" \
  --load_model_path /mmu_vcg_ssd/shiyang06/Project/Latent_Think/checkpoint/avt_sft/9.27_avt_sft_full_ce2.0 \
