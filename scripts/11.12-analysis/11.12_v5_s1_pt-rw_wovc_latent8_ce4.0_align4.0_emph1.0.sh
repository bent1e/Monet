...
SAVE_CKPT=11.12-emph-analysis_v5_s1_rw-wovc_pt-sft_gc_latent${LATENT_SIZE}_ce${CE_EMPHASIZE_FACTOR}_align-wt${ALIGNMENT_WEIGHT}_emph-wt${EMPHASIZE_LATENT_WEIGHT}
...
torchrun --nproc-per-node=2 --master-port=29502 -m src.main \
  --epochs 3 \
  --bsz 1 \
  --grad_accum_steps 16 \
  --task "mm-reasoning" \
