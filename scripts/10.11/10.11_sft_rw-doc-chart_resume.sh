...
SAVE_CKPT=10.7_sft_rw-doc-chart_ce${CE_EMPHASIZE_FACTOR}
...
torchrun --nproc-per-node=8 --master-port=29501 -m src.main \
  ...
  --log_file "./log.txt" \
  --load_model_path /mmu_vcg_ssd/shiyang06-temp/Latent_Think/checkpoint/avt_sft/10.7_sft_rw-doc-chart_ce2.0/checkpoint-3884 \
