...
SAVE_CKPT=11.11-emph-analysis_v5_s1_rw_pt-sft_gc_latent${LATENT_SIZE}_ce${CE_EMPHASIZE_FACTOR}_align-wt${ALIGNMENT_WEIGHT}_emph-wt${EMPHASIZE_LATENT_WEIGHT}_mask-latent
...
torchrun --nproc-per-node=2 --master-port=29506 -m src.main \
  --epochs 3 \
  --bsz 1 \
  --grad_accum_steps 16 \
  --task "mm-reasoning" \
