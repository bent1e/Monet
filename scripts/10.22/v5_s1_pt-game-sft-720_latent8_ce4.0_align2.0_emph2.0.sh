...
SAVE_CKPT=10.22_v5_s1_game_pt-sft-720_latent${LATENT_SIZE}_ce${CE_EMPHASIZE_FACTOR}_align-wt${ALIGNMENT_WEIGHT}_emph-wt${EMPHASIZE_LATENT_WEIGHT}
...
torchrun --nproc-per-node=8 --master-port=29501 -m src.main \
  ...
  --log_file "./log.txt" \
  --load_model_path /mmu_vcg_ssd/shiyang06-temp/Latent_Think/checkpoint/avt_sft/10.12_sft_game_ce2.0/checkpoint-720 \
