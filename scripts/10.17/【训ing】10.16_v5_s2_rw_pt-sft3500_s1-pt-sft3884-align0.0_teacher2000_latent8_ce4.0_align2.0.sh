...
TEACHER_ALIGN_WEIGHT=0.0
TEACHER_STEP=2000

TEACHER=10.15_v5_s1_rw_pt-sft_gc_latent8_ce4.0_align-wt${TEACHER_ALIGN_WEIGHT}_emph-wt2.0/checkpoint-${TEACHER_STEP}
...
SAVE_CKPT=10.16_v5_s2_rw_pt-sft3500_s1-pt-sft3884_teacher-al${TEACHER_ALIGN_WEIGHT}-${TEACHER_STEP}_latent${LATENT_SIZE}_ce${CE_EMPHASIZE_FACTOR}_align${ALIGNMENT_WEIGHT}
torchrun --nproc-per-node=8 --master-port=29501 -m src.main \
  ...
  --log_file "./log.txt" \
  --load_model_path /mmu_vcg_ssd/shiyang06-temp/Latent_Think/checkpoint/avt_sft/10.7_sft_rw-doc-chart_ce2.0/checkpoint-3500 \
