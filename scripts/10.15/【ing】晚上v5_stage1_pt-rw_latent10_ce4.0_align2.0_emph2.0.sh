...
SAVE_CKPT=10.15_v5_s1_rw_pt-sft_gc_latent${LATENT_SIZE}_ce${CE_EMPHASIZE_FACTOR}_align-wt${ALIGNMENT_WEIGHT}_emph-wt${EMPHASIZE_LATENT_WEIGHT}
...
torchrun --nproc-per-node=8 --master-port=29501 -m src.main \
  ...
  --log_file "./log.txt" \
  --load_model_path /mmu_vcg_ssd/shiyang06-temp/Latent_Think/checkpoint/avt_v5/10.15_v5_s1_rw_pt-sft_gc_latent10_ce4.0_align-wt2.0_emph-wt2.0/checkpoint-500 \
...
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7
python /mmu_vcg_ssd/shiyang06/Tool/special/video_frame_split.py