...
from verl.workers.rollout.rstar_deepthink.agents.utils import extract_and_check as rstar_extract_and_check
from tqdm import tqdm
from examples.reward_function.mcts_reward_function import extract_and_check as easyr1_mc_extract_and_check
from examples.reward_function.avt_reward_function import extract_and_check as easyr1_avt_extract_and_check
from examples.reward_function.avt_reward_function import extract_and_check_api as easyr1_avt_extract_and_check_api
from examples.reward_function.avt_reward_function import rule_then_api_batch_judge
...
class FSDPWorker(Worker):
    def __init__(
        ...
        '''self._host_embed = (
            self._is_rollout and int(os.environ.get("LOCAL_RANK", "0")) == 0
        )''' 
        if self._is_rollout and not config.rollout.mc.ablation_process_reward and config.rollout.sampling_strategy in ["mc", "mc2"]:
            self.embed_model = LLM(model=self.config.rollout.mc.embedding_model_path, task="embed", gpu_memory_utilization=0.2, dtype="bfloat16", max_model_len=2048)
    ...
    def _build_rollout(self) -> None:
        ...
        if self.config.rollout.sampling_strategy in ["mc", "mc2"]:
            self.hash_server = ray.get_actor(self.config.rollout.mc.hash_server_name)
        elif self.config.rollout.sampling_strategy in ["avt"]:
            self.hash_server = ray.get_actor(self.config.rollout.avt.hash_server_name)
        else:
        ...
        #self.embed_model = ray.get_actor(self.config.rollout.mc.embed_server_name)  if self.config.rollout.sampling_strategy in ["mc", "mc2"] else None # 取已存在的 Actor
        self.embed_tokenizer = None
        ...
        with self.rollout_sharding_manager:
            ...
            if mode == "test":
                ...
                if self.config.rollout.sampling_strategy in ["mc","mc2"]:
                    ...
                    output = self.rollout.generate_sequences_avt(prompts=prompts)
                else:
                    raise NotImplementedError(f"Sampling strategy {self.config.rollout.sampling_strategy} not supported for {mode} mode.")
            elif mode == "train_rl_gen":
                if self.config.rollout.sampling_strategy == "greedy":
                    output = self.rollout.generate_sequences(prompts=prompts)
                elif self.config.rollout.sampling_strategy == "mcts":
                    output = self.rollout.generate_sequences_mcts(prompts=prompts)
                elif self.config.rollout.sampling_strategy == "mc":
                    output = self.rollout.generate_sequences_mc(prompts=prompts)
                elif self.config.rollout.sampling_strategy == "mc2":
                    if self.config.rollout.mc.ablation_process_reward:
                        output = self.rollout.generate_sequences(prompts=prompts)
                    else:
                        output = self.rollout.generate_sequences_mc2(prompts=prompts)
                elif self.config.rollout.sampling_strategy in ["avt"]:
                    output = self.rollout.generate_sequences_avt(prompts=prompts)
                else:
                    raise NotImplementedError(f"Sampling strategy {self.config.rollout.sampling_strategy} not supported for {mode} mode.")
    ...
    def get_embed_service(self):
        """把 EmbedService 的 ActorHandle 返回给调用者（Ray 句柄可以跨进程传递）"""
    ...
    def compute_rule_based_judge(self, data: DataProto):
        ...
        if self.config.rule_based_judge.judge_function_name=="rule_then_api_batch_judge":
            ...
            for i in tqdm(range(len(data)), desc="Rule-based judge", position=position, disable=self.rank != 0):
                ...
                if "rstar_deepthink" in self.config.rule_based_judge.judge_function:
                    correctness.append(rstar_extract_and_check(response_str, ground_truth))
                elif "mcts_reward_function" in self.config.rule_based_judge.judge_function:
                    correctness.append(easyr1_mc_extract_and_check(response_str, ground_truth))
                elif self.config.rule_based_judge.judge_function == "./examples/reward_function/avt_reward_function.py:extract_and_check":
                    correctness.append(easyr1_avt_extract_and_check(response_str, ground_truth))
                elif self.config.rule_based_judge.judge_function == "./examples/reward_function/avt_reward_function.py:extract_and_check_api":
                    correctness.append(easyr1_avt_extract_and_check_api(question, response_str, ground_truth, self.api_client))
                else:
