...
class BaseConfig:
    ...
    mllm_dir: str = field(
        default="", metadata={"help": "mllm model dir"}
    )
    llm_dir: str = field(
        default="", metadata={"help": "llm model dir"}
    )
    dataset_dir: str = field(
        default="", metadata={"help": "dataset dir"}
    )
    allowed_devices: str = field(
        default="0,1,2,3,4,5,6,7", metadata={"help": "dataset dir"}
    )
    use_vllm_for_multimodal: bool = field(
        default=False, metadata={"help": "dataset dir"}
    )
    task: str = field(
        default="", metadata={"help": "task to perform. choices = [multimodal_reasoning, text_reasoning, reward_analysis]"}
    )
    rollout_save_dir: str = field(
        default="./results/MCTS_rollouts", metadata={"help": "rollout save dir"}
    )
    test_result_save_dir: str = field(
        default="./results/test_results", metadata={"help": "test result save dir"}
    )
    ...
    max_num_seqs: int = field(
        default=10, metadata={"help": "Maximum number of requests (a request is an output sequence) to handle during a single forward pass."}
    )
    llm_devices_for_vllm: str = field(
        default="", metadata={"help": "devices to load the vllm LLM"}
    )
    mllm_devices_for_vllm: str = field(
        default="", metadata={"help": "devices to load the vllm MLLM"}
    )
    do_sample: bool = field(
        default=True, metadata={"help": "whether to use sampling for generation"}
    ) # only for non-vllm
    ...
    stop_exception: Optional[dict] = field(
        default=None, metadata={"help": "these words shouldn't be the stop of a step"}
    )
    ...
    # reward analysis
    reward_analysis_max_path_len: int = field(
        default=10, metadata={"help": "the largest length of the path in reward anaysis"}
    )
    # training verifier
    layers: int = field(
        default=10, metadata={"help": "use the features from layer l, and use the parameter from l+1 as the initialization as the verifier"}
    )